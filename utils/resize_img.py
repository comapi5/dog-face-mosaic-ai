...
def resize_with_padding(image, target_size):
    ...
    if aspect_ratio > target_aspect_ratio:
        ...
        padded_image = cv2.copyMakeBorder(
            resized_image, padding, padding, 0, 0, cv2.BORDER_CONSTANT
        )
        ...
        resized_image = cv2.resize(image, (new_width, new_height))
        padded_image = cv2.copyMakeBorder(
            resized_image, 0, 0, padding, padding, cv2.BORDER_CONSTANT
        )
...
if __name__ == "__main__":
    ...
    img_paths = glob(os.path.join("..", "..", "dog_mosaic_annotation", "frames", "*"))
    assert len(img_paths) != 0, "ファイルが見つかりません。"
